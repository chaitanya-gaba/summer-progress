...
function render(state, el) {
  ...
  state.filtered.forEach(event => {
    el.grid.appendChild(createEventCard(event));
  });
...
function createEventCard(event) {
  ...
  card.innerHTML = `
    ...
    <div class="event-card-body">
      <div class="event-badge-category" style="display:none">${event.category}</div>
      ...
      <div class="event-card-footer">
        ...
        <a href="event-details.html?id=${event.id}" class="btn-primary" style="font-size:13px;padding:7px 16px" onclick="event.stopPropagation()">Book Now</a>
      </div>
    </div>
  `;
...
function startCountdowns() {
  function tick() {
    document.querySelectorAll(".event-countdown[data-date]").forEach(el => {
      ...
      if (diff <= 0) {
        el.innerHTML = `<span style="font-size:12px;color:#e11d48;font-weight:600">Live Now!</span>`;
        return;
      }
